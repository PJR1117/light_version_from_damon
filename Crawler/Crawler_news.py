...
class WebCrawlFromjrj(object):
    '''Crawl company news from 'http://stock.jrj.com.cn/xwk/' website.
    ...
    def GenDatesLst(self): # 生成滚动窗口
        '''Divide date list into parts using Range parameter.
        '''
        DatesLst = self.getEveryDay(self.startDate,self.endDate) #将日期按每天划分
        NewDatesLst = []
        k = 0
    ...
    def CrawlHistoryCompanyNews(self,datelst): # datelst为生成的日期滚动窗口
        '''Crawl historical company news
        '''
        self.ConnDB() # 联结数据库,为类实例化时输入的
        AddressLst = self.extractData(['Address'])[0]#从类实例化输入的集合中取出去重的Address字段,形成列表
        if AddressLst == []: # 如果列表为空
            urlsAndDates = []
            url_Part_1 = 'http://stock.jrj.com.cn/xwk/' # 自己给一个地址
            url_Part_2 = '_1.shtml'
            for date in datelst:
    ...
    def coroutine_run(self):
        ...
        '''
        jobs = []
        dateLst = self.GenDatesLst() # 生成日期滚动窗口
